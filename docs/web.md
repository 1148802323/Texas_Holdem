...
3. 至少两名玩家有筹码后，管理员点击“开始下一手”。首手庄位随机抽取，后续按实体座位轮转；牌桌在庄家座位前方显示 D 按钮。玩家通过牌桌操作，所有动作由服务端校验和持久化。
...
6. 退出牌桌会注销当前身份，将剩余筹码记录为离桌兑出并清除浏览器令牌；原身份的私人历史此后无法再通过该昵称访问。行动有 30 秒倒计时，超时能过牌则过牌，否则弃牌。
...
- 从旧版启动时，数据库自动迁移到支持旁观和离桌记录的 v2 结构；重启已有的服务进程后才会执行迁移。升级前可先备份 `database/holdem.sqlite3`。
