...
房间可使用自由昵称，或预设昵称列表。同一房间内昵称采用去除首尾空格及大小写折叠后的值比较；同一座位和昵称都只能被占用一次。玩家首次加入时获得随机会话令牌，数据库只保存其 SHA-256 摘要。网页接口通过 `HttpOnly` Cookie 保存令牌，并在 HTTPS 下启用 `Secure`。
...
`hands.private_snapshot` 包含底牌和剩余牌堆，是服务端恢复牌局所需的**私密数据**。玩家界面只能调用 `player_view()` 或 `hand_history()`，不能访问 `load_game()` 或数据库原始行。管理员概览由 `room_overview()` 提供；HTTP 接口在访问前验证管理员会话。
