...
已整理现有 Python 模拟引擎和演示脚本。前端、后端网络接口、数据库及部署目录目前为占位，尚未实现网络对局。本次整理仅调整文件位置与导入路径，没有修改游戏规则。
...
将游戏引擎改为可逐步推进、可保存恢复的状态机，补齐规则测试。详细顺序见 [开发计划](docs/development-plan.md)，已确认范围见 [需求说明](docs/requirements.md)。
